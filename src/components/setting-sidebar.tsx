...
export const SettingSidebar: FC<SettingSidebarProps> = ({ isVisible, onClose }) => {
  ...
  const setMessagesInContext = useChatSettings((state) => state.setMessagesInContext)
  const responseLanguage = useChatSettings((state) => state.responseLanguage)
  const setResponseLanguage = useChatSettings((state) => state.setResponseLanguage)
    ...
    >
      <header className='flex items-center justify-between gap-2 text-foreground'>
        <div>
          ...
          <p className='text-xs text-muted-foreground'>Personalise how you work with RedPill.</p>
        </div>
        <Button size='icon' variant='ghost' aria-label='Close settings sidebar' onClick={onClose}>
          <X className='size-4' />
        </Button>
      </header>
          ...
          <Card className='gap-4'>
            ...
            <CardContent className='space-y-4 pt-0'>
              <div className='space-y-1'>
                ...
                <p className='text-xs text-muted-foreground'>Switch between light or dark modes.</p>
              </div>
              <Button
                variant='outline'
            ...
            <CardContent className='space-y-6 pt-0'>
              <div className='space-y-3'>
                <div className='space-y-1'>
                  ...
                  <p className='text-xs text-muted-foreground'>How many recent messages are sent to the model (1-50).</p>
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onBlur={(event) => {
                      ...
                      const clamped = Math.min(Math.max(value, MIN_MESSAGES), MAX_MESSAGES)
              ...
              <div className='space-y-3'>
                <div className='space-y-1'>
                  ...
                  <p className='text-xs text-muted-foreground'>Choose the language preferred for AI replies.</p>
                </div>
